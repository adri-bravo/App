...
export const DatePickerMixin = subclass => class VaadinDatePickerMixin extends mixinBehaviors([IronResizableBehavior], subclass) {
  ...
  static get observers() {
    return ['_updateHasValue(value)', '_selectedDateChanged(_selectedDate, i18n.formatDate)', '_focusedDateChanged(_focusedDate, i18n.formatDate)', '_announceFocusedDate(_focusedDate, opened, _ignoreAnnounce)'];
  }
  ...
  checkValidity() {
    const inputValid = !this._inputValue || this._selectedDate && this._inputValue === this.i18n.formatDate(DatePickerHelper._extractDateParts(this._selectedDate));

    const minMaxValid = !this._selectedDate || DatePickerHelper._dateAllowed(this._selectedDate, this._minDate, this._maxDate);
